...
class RotaryPositionalEncoding(nn.Module):
    def __init__(self, dim, max_seq_len=5000):
        ...
        :param dim: 输入向量的维度，必须是偶数。
        :param max_seq_len: 支持的最大序列长度。
        """
        super(RotaryPositionalEncoding, self).__init__()
        if dim % 2 != 0:
            raise ValueError("RotaryPositionalEncoding要求输入维度为偶数。")

        inv_freq = 1.0 / (10000 ** (torch.arange(0, dim, 2).float() / dim))
        positions = torch.arange(0, max_seq_len).float()
        sinusoid_inp = torch.einsum("i,d->id", positions, inv_freq)
    ...
    def forward(self, x):
        ...
        :return: 应用RoPE后的张量，形状为(batch_size, seq_len, dim)
        """
...
# =======================
# Transformer分类模型集成RoPE
# =======================
class TransformerClassifierWithRoPE(nn.Module):
    def __init__(self, input_size, d_model, nhead, num_encoder_layers, dim_feedforward, num_classes, max_seq_len=5000):
        """
        初始化基于Transformer的分类模型，集成RoPE。

        :param input_size: 每个时间步的输入特征大小。
        :param d_model: 嵌入向量的维度。
        :param nhead: 多头注意力机制的头数。
        :param num_encoder_layers: Transformer编码器的层数。
        :param dim_feedforward: 前馈网络的隐藏层维度。
        :param num_classes: 分类数。
        :param max_seq_len: 支持的最大序列长度。
        """
        super(TransformerClassifierWithRoPE, self).__init__()
        ...
        # 旋转位置编码
        self.rotary_pos_encoder = RotaryPositionalEncoding(d_model, max_seq_len)
    ...
    def forward(self, src):
        ...
        # 应用RoPE
        src = self.rotary_pos_encoder(src)  # (batch_size, seq_len, d_model)
